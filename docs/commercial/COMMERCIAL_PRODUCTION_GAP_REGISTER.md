...
**Read GAP-019 first, then GAP-018, then GAP-000.** GAP-019 (at the very end of this document) is the latest round of fixes and the current, re-verified live state as of 2026-08-10 ~15:46 UTC. GAP-018 is a live production-correctness incident affecting `cyberdudebivash.in`'s real API, discovered the same day via a founder-provided Cloudflare dashboard screenshot — it remains the single most urgent item here and is **still not rolled back** (re-confirmed live in GAP-019). GAP-000 was found by directly testing the live production site during this audit (not inferred from source alone) and remains the most urgent *ARMY-specific* item; GAP-019 records a code-level attempt at its domain-routing half.

**⚠️ If you are reading GAP-005 in isolation: its "recommended resolution" text below is now stale and actively wrong. Do not bind `army.cyberdudebivash.in` to `cyberdudebivash-security-hub` — that Worker is the main hub's shared API (GAP-018), not ARMY's. The correct target is `cyberdudebivash-army-api`. See GAP-019.**
...
**Update (2026-08-10, ~15:46 UTC, re-verified live in this session):** re-ran the exact test from this GAP's own "Test requirement" line. Still failing the same way: `army.cyberdudebivash.in/` returns HTTP 200 with `x-github-request-id`/Fastly headers (GitHub Pages), and `army.cyberdudebivash.in/api/feed` returns HTTP 404 with the same GitHub Pages header signature — confirmed byte-identical to the finding above, nothing has changed on the routing side since this was first found. Separately, `cyberdudebivash-army-api`'s own `*.workers.dev` subdomain was re-tested directly and now serves the correct dashboard HTML *and* a correct, live `/api/feed` response (real KEV/EPSS data) — the Worker itself is fully healthy, it is purely unreachable at the public domain. Given this, `worker/wrangler.toml` now declares `army.cyberdudebivash.in` as a Worker Custom Domain (`[[routes]]`, `custom_domain = true`) so `wrangler deploy` attempts to provision the binding itself on the next deploy, instead of waiting on a dashboard click this audit cannot perform. See GAP-019 for the full writeup and how to verify whether it took effect.
...
- **Not fixed in this branch.** Reconfiguring a Git-integration build setting is a dashboard-side action this audit cannot perform.
- **⚠️ SUPERSEDED 2026-08-10 by GAP-018 — do not follow the paragraph below.** It recommended binding `army.cyberdudebivash.in` to `cyberdudebivash-security-hub`. That was written *before* GAP-018 was discovered later the same day: `cyberdudebivash-security-hub` is not a redundant/misconfigured spare, it is the **main hub's own shared API Worker** (`cyberdudebivash.in/api`, 10.8k real requests). Binding ARMY's public domain to it would not fix anything — it would point ARMY at a Worker that a founder rollback is expected to restore to the main hub's *original* (non-ARMY) code at any time, breaking ARMY again the moment that rollback happens. **Corrected resolution: the target is `cyberdudebivash-army-api`**, not `cyberdudebivash-security-hub` — see GAP-019.
- ~~**Recommended resolution (given to the founder directly):** don't try to repair `cyberdudebivash-army-api` — disconnect/delete it and confirm `army.cyberdudebivash.in`'s custom domain is bound to `cyberdudebivash-security-hub` (the service `wrangler.toml` actually declares, with the correct KV binding, already published successfully by the working `deploy-worker.yml` GitHub Actions job). That collapses three competing deploy paths down to one correct one instead of trying to fix a redundant, misconfigured second/third path.~~ *(struck through, not deleted, so the reasoning error is visible rather than silently rewritten — see the superseded note above.)*
- **What's still actually true from this entry and still needs founder action:** if Cloudflare's native "Workers Builds" Git integration for `cyberdudebivash-army-api` is still connected, it independently rebuilds on pushes with build settings that (per the founder's dashboard screenshot) don't find `worker/wrangler.toml` and fall back to a static-assets-only publish with zero bindings — which would silently undo the correct GitHub-Actions-deployed script (routes, KV binding, cron) the next time it fires. This audit cannot see or disconnect that integration. **Action needed: Cloudflare dashboard → Workers & Pages → `cyberdudebivash-army-api` → Settings → Builds → disconnect/pause the Git integration**, so the only thing deploying this Worker is the GitHub Actions workflow in this repo (`deploy.yml`, see GAP-005a and GAP-019).
...
**Severity:** P1 · **Status: [RESOLVED in this branch — evidence-backed, not guessed]**
**Resolution (2026-08-10, GAP-019 round):** this was previously left undecided because guessing which secret pair was populated risked killing the one working deploy path. That's no longer a guess: GitHub Actions run history for both workflows was pulled directly and compared run-by-run. Both `deploy-worker.yml` (`CLOUDFLARE_API_TOKEN`) and `deploy.yml`'s `deploy-worker` job (`CF_API_TOKEN` + `CF_ACCOUNT_ID`) show **`conclusion: success` on every push since at least 2026-08-09**, including the same PR #6 merge commit (`99e6c2ff`) one second apart (15:27:05 and 15:27:06 UTC) — i.e. both secret pairs are real, populated, and valid, and both were redeploying the identical `worker/` directory every time (harmless duplication, not a conflict, since there is only one `wrangler.toml`/`src/index.js` for either to deploy). Kept `deploy.yml`'s job (test-gated — a failing `python test_hotfix.py` blocks it; explicit secret-presence validation step) and deleted `deploy-worker.yml` (unconditional, no test gate, plain `npm install -g wrangler`). This removes one of the two redundant Worker-deploy runs on every push and the CI-log confusion that comes with it — it does not change which code gets deployed, since both were deploying the same thing.
...
**Update (2026-08-10, ~15:46 UTC, re-verified live in this session):** re-ran the exact test above. **Still not rolled back.** Both `curl https://cyberdudebivash.in/api/health` and a direct request to `cyberdudebivash-security-hub`'s own `*.workers.dev` subdomain return the identical ARMY payload (`"version":"200.0-standalone","source":"CISA KEV + FIRST.org EPSS (direct, no cyberdudebivash.in dependency)"`), timestamped seconds apart from each other, confirming they are still the same live Worker instance. Nothing in this session's code changes touches or can touch this — `cyberdudebivash-security-hub` has not been this repo's deploy target since PR #6 (confirmed: this repo's CI only ever targets `cyberdudebivash-army-api` now), so there is no further commit that moves this forward.
**Update (2026-08-10, ~16:15 UTC, exact rollback target identified from real evidence, not a timestamp guess):** the founder pasted the Cloudflare dashboard's Deployments/version-history view for `cyberdudebivash-security-hub` (1,035 total versions). Cloudflare gives each version its own unique preview subdomain (`https://<version-id>-cyberdudebivash-security-hub.<account>.workers.dev/`), so instead of guessing from relative timestamps, each of the 10 most recent versions was curled directly and its actual response inspected:
- `1803da25`, `42d19cad` (1h ago): ARMY `v200.0-standalone` — this session's code.
- `27036d90` through `d7df3099` (4 versions, 1h–5h ago): an *earlier* ARMY deploy, `v185.1`, stuck serving `{"maintenance":true,"feed":[]}` — i.e. the shared-Worker contamination predates this session's PR #5 and traces back to the "EMERGENCY: fix worker" commits from 2026-08-09, not just this audit's own work.
- **`b22d872b` (11h ago): the real main hub** — `{"service":"CYBERDUDEBIVASH AI Security Hub","version":"40.0.0","company":"CyberDudeBivash Pvt. Ltd.",...}`, full company/contact/telegram/tools-subdomain detail, categorically different shape from every ARMY response above it.
- `f9e2b1ff` (12h ago): same real main hub, one step further back than necessary.

**`b22d872b` is the exact, evidence-confirmed rollback target** — the most recent version that is genuinely the main hub's own API, sitting immediately at the boundary before the ARMY contamination begins. **Action: Workers & Pages → `cyberdudebivash-security-hub` → Deployments → `b22d872b` (11h ago) → Rollback to this deployment.**
**New risk surfaced by this check:** the two most recent `v200.0-standalone` versions (1h ago) were tagged **"Manually deployed... by iambivash.bn"** — not attributed to GitHub Actions. This repo's CI has only ever targeted `cyberdudebivash-army-api` since PR #6 (confirmed repeatedly in this register), so these two manual deploys came from somewhere else: a local `wrangler deploy` run against a `worker/` folder whose `wrangler.toml` still said `cyberdudebivash-security-hub` — most plausibly a stale local clone predating PR #6, or a leftover from Kimi's earlier session (see the Kimi-log forensic entries above). **This means a rollback alone will not hold** if that same local source runs `wrangler deploy` again. Whoever has local Wrangler access to this Worker needs to stop deploying from any copy of this repo's `worker/` directory unless its `wrangler.toml` is first confirmed to say `cyberdudebivash-army-api`.
**Test requirement (updated):** after rolling back to `b22d872b`, `curl https://cyberdudebivash.in/api/health` should show `"version":"40.0.0"` and the real company/service fields above — not `200.0-standalone` and not a `maintenance` flag.

---

### GAP-019 — Follow-up round: attempted a code-level fix for the domain-routing half of GAP-000, resolved GAP-005a with evidence, and corrected a now-dangerous cross-reference in GAP-005
**Severity:** P0 (carries GAP-000's domain-routing severity) · **Status: [CONFIRMED root cause via real deploy log — one manual DNS action remains, no further code fix possible]**
**Context:** Responding directly to the founder's "we are still blocked... let's fix the existing issues & let's build" after PR #6. Before making any change, re-verified live state fresh rather than relying on earlier findings, since two prior rounds of fixes had already merged:
- `army.cyberdudebivash.in/` → HTTP 200, but headers (`x-github-request-id`, Fastly `x-served-by`/`via`) prove it's **GitHub Pages**, not the Worker.
- `army.cyberdudebivash.in/api/feed` → HTTP 404, GitHub Pages' own 404 page, same header signature.
- `cyberdudebivash-army-api.<account>.workers.dev/` and `/api/feed` → **both fully healthy**, correct dashboard HTML and correct live KEV/EPSS data. The Worker itself has no defect; it is simply never reached at the public domain.
- `cyberdudebivash.in/api/health` and `cyberdudebivash-security-hub.<account>.workers.dev/api/health` → both still return ARMY's payload. GAP-018's rollback has **not** happened yet.
- GitHub Actions run history for both Worker-deploy workflows, pulled and compared directly (not assumed): both `deploy-worker.yml` and `deploy.yml`'s `deploy-worker` job have been succeeding on every push since 2026-08-09, proving both secret pairs are valid and both were harmlessly double-deploying the same code — this de-risked consolidating them (see GAP-005a).
- No `CLOUDFLARE_API_TOKEN`/`CF_API_TOKEN`/`CF_ACCOUNT_ID` or any Cloudflare credential exists in this audit session's own environment — confirmed by checking. This audit cannot call the Cloudflare API or dashboard directly under any circumstance; every Cloudflare-side fact in this register comes from live HTTP probes against public endpoints or from founder-provided screenshots, never from privileged access.
**What was changed:**
1. `worker/wrangler.toml` — added `[[routes]] pattern = "army.cyberdudebivash.in" custom_domain = true`. This is a real wrangler/Cloudflare feature: declaring a route with `custom_domain = true` asks Cloudflare, on the next successful `wrangler deploy`, to provision that hostname as a Worker Custom Domain — which includes creating/repointing its DNS record and giving the Worker exclusive edge-level ownership of the hostname, ahead of whatever GitHub Pages is currently serving. This is an attempt to fix GAP-000's domain-routing half **without** needing dashboard access, using the same CI credentials that have already been proven (via run history) to successfully deploy this Worker's code.
2. Deleted `.github/workflows/deploy-worker.yml` (see GAP-005a — resolved with evidence, not guessed).
3. Corrected GAP-005's now-dangerous stale recommendation (see that entry) before it could be followed and make GAP-018 worse.
**Update (2026-08-10, ~16:09 UTC, confirmed via real post-merge deploy log — `get_job_logs` on the `deploy-worker` job, run 31407370416, job 93516935615, not speculation):** PR #7 merged, `deploy.yml`'s `deploy-worker` job ran for real for the first time (previously always `skipped` on PR-branch pushes) and **failed** — but the failure is precise and informative, not a dead end:
```
Total Upload: 14.86 KiB / gzip: 5.17 KiB
Your worker has access to the following bindings:
- KV Namespaces: THREAT_INTEL_KV: 4778dfcff095404c9ba4eec59e658c3e
- Vars: CORS_ORIGIN: "https://army.cyberdudebivash.in"
Uploaded cyberdudebivash-army-api (1.69 sec)

✘ [ERROR] A request to the Cloudflare API (/accounts/***/workers/scripts/cyberdudebivash-army-api/domains/records) failed.
  Hostname 'army.cyberdudebivash.in' already has externally managed DNS records (A, CNAME, etc). Delete them first or try a different hostname. [code: 100117]
```
Two important facts this proves: (1) the script/KV/vars deploy **succeeded** — this Worker is correctly and fully deployed with the right bindings, only the domain step failed; (2) this is **not** a token-permission problem — the CI's `CF_API_TOKEN` successfully authenticated and reached the Custom Domains API and got a structured, well-formed conflict response back (`code: 100117`), not an auth/403 error. The blocker is that `army.cyberdudebivash.in` already has a DNS record Cloudflare's Custom Domains system doesn't own (the record pointing at GitHub Pages, consistent with everything else found in this GAP and GAP-000) — Custom Domains refuses to silently take over a hostname with pre-existing, externally-managed DNS records.
**The one remaining action, now fully specified, dashboard-only, no code can do this:** Cloudflare dashboard → `cyberdudebivash.in` zone → DNS → Records → delete the existing record for `army` (the one resolving to GitHub Pages) → then either wait for the next push touching `worker/**`, or re-run the failed `deploy-worker` job directly from the GitHub Actions UI (Actions tab → the failed run → "Re-run failed jobs") — no new commit needed. Once the conflicting record is gone, this same deploy step will succeed in creating the Custom Domain's own DNS record, and `army.cyberdudebivash.in` will start reaching the Worker.
**Also worth doing at the same time, not strictly blocking:** clear the GitHub Pages custom domain setting for this hostname (repo Settings → Pages → Custom domain → remove) so nothing is left pointing at the old, now-orphaned configuration.
**Still true and still a real risk:** if Cloudflare's native "Workers Builds" Git integration (GAP-005) is still connected to `cyberdudebivash-army-api`, it fires independently of this repo's GitHub Actions and was screenshot-confirmed to publish a static-assets-only build with zero bindings — which would strip the route/KV/cron this fix and GAP-008 both depend on the next time it runs. Disconnecting it (Cloudflare dashboard → `cyberdudebivash-army-api` → Settings → Builds) remains a precondition for this fix holding permanently, not optional cleanup.
**Evidence:** Live `curl`/DNS probes performed in this session; GitHub Actions run history for both workflows; the actual `deploy-worker` job log quoted above in full (not summarized or guessed); `worker/wrangler.toml` diff; environment variable check for Cloudflare credentials (none found, confirming this audit has never had privileged Cloudflare access at any point).
**Dependencies:** One Cloudflare DNS dashboard action (delete one record) — this is now the single blocking dependency, precisely identified.
**Test requirement:** After the DNS record is deleted and the job re-run/re-triggered, `curl -sI https://army.cyberdudebivash.in/api/feed` — success is `content-type: application/json` with **no** `x-github-request-id` header.
**Rollback requirement:** Remove the `[[routes]]` block from `worker/wrangler.toml` and redeploy — trivial, and safe, since the failed custom-domain provisioning attempt did not touch or revert the currently-serving GitHub Pages content (confirmed: `army.cyberdudebivash.in` was re-tested after this failure and is still serving GitHub Pages exactly as before, `age: 0`/`x-cache: MISS` fresh headers, not a stale cached success).
...
| GAP-005 | Three independent Cloudflare deploy mechanisms; native Workers Builds confirmed serving static-assets-only, zero bindings | P1 | DOCUMENTED (stale "bind to security-hub" advice corrected — see GAP-019) |
| GAP-005a | Two GitHub-Actions-side deploy workflows also conflict | P1 | **RESOLVED** (evidence: both proven valid via run history; consolidated to one) |
...
| GAP-018 | **This repo's Worker deploy target was shared with the main hub's real API — PR #5 replaced its live behavior** | **P0** | **FIXED (code) / founder must roll back the live deployment — re-confirmed STILL NOT done as of 15:46 UTC** |
| GAP-019 | Attempted code-level Custom Domain fix for GAP-000; resolved GAP-005a with evidence; corrected dangerous stale GAP-005 advice | P0 | **Root cause CONFIRMED via real deploy log — blocked on one DNS record deletion (dashboard-only)** |

**Zero P0/P1 findings were resolved by silently editing customer-facing pricing or claims.** Per the task's governance rules, GAP-001, GAP-002, and GAP-003 require a business, staffing, or credentials-access decision this audit is not positioned to make, and are escalated explicitly rather than guessed at. **Three dashboard-only actions remain the founder's first priority, in this order — every one of them has now been narrowed to a single, precisely-specified click or deletion, not an open-ended investigation:**
1. **GAP-018 — roll back `cyberdudebivash-security-hub`'s live deployment.** Confirmed, freshly, still not done. Nothing in this repo can do this; the pre-incident code was never in this repository. Workers & Pages → `cyberdudebivash-security-hub` → Deployments → roll back to before 2026-08-10 ~14:42 UTC.
2. **GAP-019 — delete the existing DNS record for `army.cyberdudebivash.in`.** Confirmed via a real deploy log (not a guess) that this is the *only* thing blocking the Worker's Custom Domain from being provisioned: Cloudflare dashboard → `cyberdudebivash.in` zone → DNS → Records → delete the `army` record → re-run the failed `deploy-worker` GitHub Actions job (no new commit needed). The Worker script itself is already correctly deployed with the right bindings; only the domain hookup is waiting on this.
3. **GAP-005's native Workers Builds Git integration for `cyberdudebivash-army-api`** — disconnect it if still connected, so only this repo's GitHub Actions workflow deploys that Worker. Without this, action #2 above may get silently undone by a static-assets-only rebuild the next time the native integration fires.

Everything else that was actually reachable from source in this repository — the client-side crash (GAP-000), the shared-Worker rename (GAP-018 code side), the ARMY data pipeline's independence from `cyberdudebivash.in` (GAP-015), the severity-floor bug (GAP-016), the duplicate deploy workflow (GAP-005a), and the domain-routing fix now blocked on exactly one DNS deletion (GAP-019) — has been fixed and shipped, verified against real live traffic and real deploy logs at every step, not assumed.
